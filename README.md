...
- [Using it](#using-it)
  ...
  - [Pinning a color](#pinning-a-color)
  - [Chroma](#chroma)
...
**Rolls palettes that are actually readable.** Every generated color is
...
terminal color between 4.5 and 9.5. The ceilings matter as much as the floors:
...
active-border gradient, a terminal with the sixteen ANSI colors in it, and a
...
preview and every swatch stay fully opaque, so the colors you are judging are
...
| **ImageMagick** | required for images | Reading colors out of a picture, and drawing backgrounds. Already on a standard Omarchy install. Without it the palette editor still works and the image half greys out with a reason. |
...
| `Ctrl+P` | open the color wheel on the selected swatch |
| `Ctrl+S` | save |
| `Ctrl+Enter` | save **and** apply |
| `Esc` | close (or close the color wheel, if it is open) |
...
theme's ground is unreadable as a light theme's, so the colors have to be
...
every other color against them.
...
The sixteen terminal colors keep their names — a roll's red is always a red —
...
one of the twenty-six is drawn from the whole color cube, with no contrast
...
Locked colors are left alone either way.

### The three that drive everything

Twenty-six colors come out of three, plus mode and chroma:
...
color wheel: a hue ring around a saturation/lightness square. It is HSL rather
than the HSV most wheels use, so it agrees with the sliders beside it about what
every number means.

A color you pick lands exactly where you put it. The solver places the colors
...
time. Click it and the color wheel opens on that color, exactly as if you had
found the row and pressed the big swatch.

Two parts wear more than one color and pick by where you point: the active
...
leaves a locked color exactly as it is. Three ways to do it:

- the dot beside a color in the grid (it appears when you hover the row; a
  right-click on the row does the same)
- the **lock** button beside the color's name above the sliders
- a right-click on anything in the preview

A locked color shows a filled yellow dot and the label `LOCKED`. Editing a
color locks it too, so a color you have set by hand is never rolled away.
...
Switching **dark** / **light** drops every lock, because a color chosen against
one ground is meaningless against the other.

That is what lets you keep rolling after you have fixed one color by hand.

### Chroma

How colorful the sixteen terminal colors are, from washed-out to vivid. It
does not affect the ground, the text or the accent.

### The twenty-six, and what the numbers mean

The grid shows every key, its color, and its **WCAG contrast ratio against the
...
| the thirteen ANSI colors | **4.5 – 9.5** | Readable as text, without one of them screaming. |
...
any of them from a terminal. Every color arrives exactly as it is on disk
...
- **Translucency** — how much of your wallpaper reads through the window. The
  preview and every swatch stay fully opaque whatever you set, so the colors you
...
- **True random roll** — off by default. On, every roll draws all twenty-six
  from the whole color cube with no contrast solving; the warning beneath the
  toggle says what that means. Locked colors are left alone.
...
It deliberately cannot roll or save a theme headlessly. All the color maths
...
**A color is stuck where I didn't put it.** The contrast solver placed it. Pin
...
(MIT) — a dark surface, a border in the theme's Hyprland active-border color,
